...
fun MapView(
    ...
    var clickedCoord by remember { mutableStateOf(LatLng(47.4979, 19.0402)) }
    var previousPlaces by remember { mutableStateOf<List<Place>>(emptyList()) }

    val cameraPositionState = rememberCameraPositionState {
        position = CameraPosition.fromLatLngZoom(LatLng(47.4979, 19.0402), 12f)
    }
    ...
    if (showAdd) {
        mainViewModel.fetchPlacesByCategory(CategoryType.ENTERTAINMENT) { places ->
            ...
            AddNewPlaceDialog(
                latLng = clickedCoord,
                onAddPlace = { placeTitle, placeText, placeLiked, categorySelected, date, atmosphere, recommend, rank, imageUri ->
                    mainViewModel.uploadPlace(clickedCoord, placeTitle, placeText, placeLiked, categorySelected, date, atmosphere, recommend, rank, imageUri)
                    showAdd = false
                },
...
fun PlaceCard(
    ...
    val likeIcon: ImageVector = when (place.likeType) {
        LikeType.YES -> Icons.Filled.ThumbUp
        LikeType.NO -> Icons.Filled.ThumbDown
        LikeType.NEUTRAL -> Icons.Filled.ThumbsUpDown
    }
